...
| ----------------------------- | --------------------------------------- | ------------------: | ------------: |
| Model Gateway                 | LiteLLM                                 |             Adopted |          High |
| Local Runtime — Windows       | Ollama                                  |             Adopted |          High |
| Local Runtime — macOS         | oMLX, with Ollama fallback              |             Adopted |          High |
...
| Current candidate / implementation | LiteLLM.                                                                                                                                                                            |
...
| Selection criteria                 | Provider support, local runtime support, frontier-provider support, browser UI compatibility, CLI compatibility, maintainability, ease of local deployment and ease of replacement. |
...
| Current candidate / implementation | oMLX as the preferred Apple Silicon runtime, with Ollama as the local fallback.                                                                                      |
...
| Current status                     | Adopted. oMLX is preferred for `macos-work`; Ollama remains the fallback.                                                                                            |
...
| Contract                           | Must support self-hosted operation, gateway-only model access, repeatable startup and recovery, profile-scoped persistence, localhost binding and clear storage behaviour.      |
| Current candidate / implementation | LibreChat v0.8.7 with MongoDB 4.4.18.                                                                                                                                           |
| Profile considerations             | The UI uses profile-scoped Compose projects, environment files and MongoDB volumes. Work and personal conversation state must not be shared accidentally.                       |
| Selection criteria                 | Gateway compatibility, container support, usability, persistence behaviour, configuration model, privacy controls, rebuildability and operational recovery.                     |
| Replacement trigger                | LibreChat becomes difficult to maintain, bypasses the gateway, handles profile data inadequately, loses required functionality or a materially better evaluated option emerges. |
| Current status                     | Adopted. Validated for `macos-work` with LiteLLM as its only model endpoint.                                                                                                    |
