...
fit: Strong fit
reason:
  - Uses LiteLLM as its only model endpoint
  - Provides a familiar browser chat workflow
  - Preserves conversations through container recreation
  - Supports profile-scoped configuration and storage
status: Adopted
```
...
| Chat UI          | LibreChat or another deliberately evaluated equivalent |
...
| Key tests         | Can CLI and browser UI workflows both use it? Can it support macOS and Windows profile policies?        |
...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Capability             | Chat UI                                                                                                                                                       |
| Current implementation | LibreChat v0.8.7 with MongoDB 4.4.18                                                                                                                          |
| Why selected           | Provides a familiar self-hosted browser UI, integrates through LiteLLM, persists conversations and supports profile-scoped deployment.                        |
| Must support           | Gateway-only model access, local and frontier aliases where policy permits, repeatable setup, localhost binding and clear persistence behaviour.              |
| Validated behaviour    | Model discovery, synthetic prompting, gateway enforcement, failure without bypass, recovery after sleep, conversation persistence and profile-scoped storage. |
| Risks                  | Operational dependency on Podman and MongoDB, upstream configuration drift, and additional local resource usage.                                              |
| Decision record        | `docs/tool-evaluations/002-browser-chat-ui.md` and ADR 0017.                                                                                                  |
